    ...
    <main className="text-white">
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12">
            <div className="mb-6 h-4 w-32 animate-pulse bg-white/10" />
            <div className="flex flex-wrap gap-3">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-12 w-24 animate-pulse bg-white/10" />
              ))}
            </div>
          </div>
          <div className="grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-[#08080c] p-7">
                <div className="mb-6 h-5 w-20 animate-pulse bg-white/10" />
                <div className="mb-4 h-8 w-3/4 animate-pulse bg-white/10" />
                <div className="mb-4 h-12 w-full animate-pulse bg-white/10" />
